...
kable(aggregate_table, col.names = c("Title", "Revenue", "Budget", "Avg Vote Out of 10")) %>% 
kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = FALSE)
```
This table exhibits the top 25 movies based on revenues, depicting the maximum money a movie is able to generate. In addition, budget and votes are displayed in order to supplement, in general, the movies' success. Consistently, the top 25 movies demonstrate high revenues, with typically a more costly budget and higher ratings!
...
This chart has shown the top 10 most common single and mixed genre for all kinds of movies that have been released before. The chart is to reflect the variance of different types genres with the change of number of movies. I made it into an order start from the least common genres to the most common genres in the top 10 popular genres because I want readers to quickly see the order of the most popular 10 types of genres in an order.
...
- What's obvious at a glance is that Drama is the most common genre type. Comedy is the second popular genre type and Documentary is the third common genre type in all movies.

- There is an obvious gap between the top three popular genre type compared to the rest seven types. The rest seven types lie between 1000 to 2000. However, the top three lie above 4000.
...
- Vast majority movies are not that profitable regardless of run time without taking into account currency inflation and time period. Some movies are seen to be following the correlation in terms of longer movies being more profitable in revenues and medium length movies generating fair amounts of revenue.
